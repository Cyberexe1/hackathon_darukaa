...
export function KpiCard({
  ...
  const displayValue = staticValue ?? `${formatValue ? formatValue(value) : value.toLocaleString()}${suffix}`;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-space-md">
        <span className="font-label-technical text-label-technical text-on-surface-variant uppercase">{label}</span>
      ...
      <div className={`font-headline-lg text-headline-lg leading-none ${tone === 'accent' ? 'text-surface-tint' : 'text-primary'}`}>
        {displayValue}
      </div>
      {description && (
        <p className="font-body-sm text-body-sm text-on-surface-variant mt-space-xs">{description}</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="bg-surface-container-lowest p-space-md rounded-xl shadow-sm flex items-center gap-space-sm">
      ...
      <div className="min-w-0">
        <div className="font-label-technical text-label-micro text-on-surface-variant uppercase truncate">{label}</div>
